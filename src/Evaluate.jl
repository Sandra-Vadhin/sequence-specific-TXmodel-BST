...
function _evaluate_opt(model::Dict{String,Any}; 
    ...
    p[6] = input;
    # setup the solver -
    prob = ODEProblem(_balances_opt, xₒ, tspan, p; saveat = Δt)
    soln = solve(prob,CVODE_BDF(),abstol=1e-9,reltol=1e-9,maxiters=1000)
...
# PUBLIC BELOW
function evaluate_opt(model::BSTModel; tspan::Tuple{Float64,Float64} = (0.0,20.0), Δt::Float64 = 0.01, 
    ...
    try
        ...
        return _evaluate_opt(internal_model_dictionary, tspan = tspan, Δt = Δt, input = input); 
