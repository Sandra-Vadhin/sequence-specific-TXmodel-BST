...
function _evaluate_positive(model::Dict{String,Any}; 
    ...
    soln = solve(prob, isoutofdomain = (u,p,t)->any(x->x<0,u))
...
"""
function evaluate_positive(model::BSTModel; tspan::Tuple{Float64,Float64} = (0.0,20.0), Δt::Float64 = 0.01, 
    input::Union{Nothing,Function} = nothing)::Tuple{Array{Float64,1}, Array{Float64,2}}

    try
        # convert the model object to the internal_model_dictionary -
        internal_model_dictionary = _build(model);
        return _evaluate_positive(internal_model_dictionary, tspan = tspan, Δt = Δt, input = input); 
